$(document).ready(function(){
	...
	function traceroutes(jj){
		var t = []; // keep track of individual dot locations
		...
		for ( var i in jj ){
			...
			// check for dupe locations, separate them visually
			
			
			// car or plane
			var tripType;
			if ( jj[i]['type'] == 'plane' ){
				tripType = '../icons/plane-map-icon-black.png'
			} else {
				tripType = '../icons/dot-green-lt.png'
			}
			var marker = new google.maps.Marker({
				...
				icon: {
					url: tripType,
//					url: '../icons/' + jj[i]['type'] + '-map-icon-black.png',
					size: new google.maps.Size(18,18),
					scaledSize: new google.maps.Size(18,18),
					origin: new google.maps.Point(0,0),
					anchor: new google.maps.Point(9,9),
			...
			// add an infowindow for each airplane
			google.maps.event.addListener(marker, 'click', (function(marker,jtrip) {
				return function() {
			...
			$('#jtrips')
				.append( 
					$('<li />').html(
						...
						'<div>' + jj[i]['Reason for trip'] + '<br />' + jj[i]['Court paid'] + '</div>'
